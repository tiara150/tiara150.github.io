...
```{r Untar files werkt wel maar files zijn niet tared of hij heeft het al gedaan}
#To untar more files (from different geo_datasets doesn't work yet)
files_tar <- list.files(path = "GSE150646")
untarr <- map(
  .x = files_tar,
  .f = untar,
)
...
list_for_unzipping <- list.files(path = "GSE150646")
...
```{r Unzip files werkt wel maar kan niet herhaald worden als de bestanden al bestaan}
files_zip <- list.files(path = "GSE150646", full.names = TRUE, pattern = ".gz")
unzipp <- map(
  .x = files_zip,
  .f = GEOquery::gunzip
)
...
```{r pheno and meta data begrijp niet waarom dit perse in tibble moet}
...
load_csv_data <- list.files(path = "GSE150646", full.names = TRUE, pattern = ".csv")
...
head(raw_counts_csv_data)
    ##make rowdata_csv waar is dit voor????????
